...
- 학습을 거쳐 최종 모델이 선정, 이 모델을 사용하여 새로운 데이터에 대해 예측
- 변수의 타입에 따라 **분류**와 **회귀**로 구분

#### 분류

- 범주형(불연속적이고 유한한 값의 집합) 종속변수를 예측하는 알고리즘을 학습시키는 행위
- 이진 분류가 대표적(대표적으로 스팸메일)
- 세 가지 이상의 범주로 분류하는 경우를 **다중 클래스 분류**라고 함
...
- 사람들의 명시적/암시적 선호도를 연구, 선호할 만한 추천 아이템을 도출
  - 영화 추천, 상품 추천

### 비지도 학습

- 주어진 데이터셋에서 특정 패턴을 찾거나 숨겨진 구조적 특징을 발견
- 지도 학습과 가장 큰 차이는 예측 대상이 되는 종속 변수가 필요없다는 것
  - 이상징후 탐지, 사용자 세분화, 토픽 모델링

### 그래프 분석

- vertex와 edge를 지정하는 구조에 대한 연구
  - 사기거래 예측, 네트워크 특성 분류, 웹 페이지 추천

### 고급 분석 프로세스

1. 분석 주제와 관련된 주제 수집
2. 데이터 파악을 위해 **데이터 정제 및 검토**
  - 데이터 정제 및 검토
  - EDA(Exploratory Data Analysis)라고도 함
3. 알고리즘이 데이터를 잘 인식하고 활용할 수 있도록 **feature engineering 수행**
  - 데이터셋 수집 및 정제가 완료되면 **머신 러닝 알고리즘에 적용 가능한 형식으로 데이터를 변환**
  - 피처 엔지니어링의 결과는 머신러닝 애플리케이션의 성과에 큰 영향을 미치므로 신중하게 수행
  - 피처 엔지니어링 프로세스
    - 데이터 정규화
    - 다른 변수들과 상호작용을 나타내는 새로운 변수 추가
    - 범주형 변수 조작 및 머신러닝 모델에서 인식할 수 있도록 적절한 형식으로 변환
  - MLlib에서 실젯값의 형태와 관계 없이 모든 변수가 실수(double)형 벡터로 입력되어야 함
4. 후보 모델을 생성하는 하나 이상의 알고리즘을 학습시키기 위해 **전체 데이터의 일부로 학습**
  - 입력 데이터에 대한 모델 정확도에 따라 내부 파라미터 수정
  - 데이터셋을 train set과 test set으로 구분하여 학습
5. 생성된 모델을 동일한 데이터에서 학습에 사용되지 않은 부분(테스트셋)에 적용하여 결과 측정, **성공 기준과 비교하여 모델 평가**
  - test set으로 테스트 실시, over-fitting과 under-fitting 여부 확인
  - 학습에 영향을 주는 다양한 하이퍼파라미터를 시험, 변형된 모델의 적합성 테스트 
6. 확보한 모델을 통하여 예측을 수행하거나 이상현상을 감지
...
- 데이터 수집과 정제
- 특징 추출과 선택
- 대규모 데이터 지도 및 비지도 학습 머신러닝 모델 학습, 튜닝
- 운영 환경 인터페이스
- `org.apache.spark.ml`에는 DataFrame 사용 인터페이스 포함
  - 머신러닝 파이프라인 구축을 위한 고급 인터페이스 제공
  - 하위 패키지인 `org.apache.spark.mllib`은 스파크 RDD API를 위한 인터페이스 제공

#### MLlib을 언제 그리고 왜 사용해야할까

- 단일 머신으로 기반으로 머신러닝을 수행할 수 있는 도구는 매우 다양
  - 학습할 수 있는 데이터 크기나 처리 시간 면에서 한계가 존재
- MLlib은 상호보완적이며 확장성 문제에 마주하게 되는 경우 스파크로 해결 가능
- 스파크의 확장 기능이 적용되는 두 가지 주요 활용 사례
  - 스파크로 데이터를 전처리하고 특징을 생성하여 대량의 데이터로부터 학습 및 테스트를 위한 데이터셋 생성에 걸리는 시간을 줄인 후 단일 머신 기반의 학습 라이브러리를 활용하여 주어진 데이터셋 학습
  - 입력 데이터나 모델 크기가 단일 머신에 올려놓기 너무 어렵거나 불편할 경우 스파크를 사용하여 분산 처리 기반의 머신러닝 수행

#### 고수준 MLlib 개념

- MLlib에는 변환자, 추정자, 평가기(evaluator), 파이프라인과 같은 구조적 유형 존재
  - `변환자`: 원시 데이터를 다양한 방식으로 변환하는 함수
    - 새로운 상호작용 변수 생성, 컬럼 정규화, 타입 캐스팅 등
    - DataFrame으로 입출력
  - `추정자`: 데이터 초기화
    - 사용자가 데이터로부터 모델을 학습시키기 위해 사용하는 알고리즘
  - 평가기: 수신자 조작 특성(ROC, Receiver Operating Characteristic) 곡선처럼 지정한 기준에 따라 어떻게 작동하는지 확인
- 저수준 데이터 타입
  - `Vector`가 일반적
  - 참고로 RDD는 mllib, DataFrame은 ml 패키지에 해당
- 하이퍼파라미터
  - 모델 아키텍처 및 일반화(regularization)와 같은 학습 프로세스에 영향을 주는 설정 매개변수
    - 학습 시작 전 설정
  - 통계모델에서 모수를 의미하는 용어도 파라미터이므로 구별하기 위해 하이퍼파라미터 라고 함
- 표준화, 정규화, 일반화
  - 표준화(stadardization): 평균을 기준으로 관측값들이 얼마나 떨어져있는지 재표현하는 방법
  - 정규화(normalization)
    - 데이터의 범위를 바꾸는 방법
    - 데이터 분포의 중심을 0으로 맞추고 분포가 특정 범위 안에 들어가도록 조정하고, 표준화하고, 모든 값을 0~1사이 값으로 재표현(min-max scale)
  - 일반화(regularization)
    - 모델 오버피딩을 방지하기 위함
    - 모델 표현식에 추가적인 제약 조건을 걸어 모델이 필요 이상으로 복잡해지지 않도록 조정
    - 릿지 회귀, 라쏘, 엘라스틱넷, 최소각 회귀(LAR) 등이 대표적 일반화 알고리즘

코드는 생략함.

## 챕터 25. 데이터 전처리 및 피처 엔지니어링

### 사용 목적에 따라 모델 서식 지정

- 분류 및 회귀 알고리즘의 경우 데이터를 Double 타입으로 캐스팅하고 Vector 타입(dense or sparse)의 컬럼을 사용하여 특징을 나타냄
- 추천 알고리즘의 경우 데이터를 사용자 컬럼, 아이템 컬럼, 등급 컬럼으로 표현
- 비지도 학습 알고리즘의 경우 입력 데이터로 사용할 특징을 Vector 타입컬럼으로 표현
- 그래프 분석은 정점과 에지가 각각 DataFrame으로 구성되어야 함
- **다양한 형태의 데이터를 확보하는 가장 좋은 방법은 변환자(데이터 입출력이 DataFrame)를 사용하는 것**

### 전처리 추정자

- 추정자는 수행하려는 변환이 입력 컬럼에 대한 데이터 또는 정보로 초기화되어야 할 때 필요
- 특정 입력 데이터에 따라 구성되는 변환자
- **단순 변환을 위해 일반 변환자 유형**과 **데이터에 따라 변환을 수행하는 추정자 유형**으로 존재

분류, 회귀, 지도, 비지도, 그래프 분석 등 생략

## 챕터 33. 에코 시스템과 커뮤니티

### 스파크 패키지 사용하기

1. 스칼라나 자바를 사용하는 경우 빌드 파일의 의존성 부분에 포함
2. 파이썬이나 R은 사용자 애플리케이션 런타임에 명시

```scala
resolvers += "bintray-spark-packages" at "https://dl.bintray.com/spark-packages/maven" // 스파크 패키지 저장소 의존성 정의
...
```shell
# spark-shell 명령에 인수를 지정하여 런타임에 패키지 추가
